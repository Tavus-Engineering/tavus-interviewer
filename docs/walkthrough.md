...
The role shown in the UI comes from the active preset's `title` in `config/presets.config.json`, or from the persona on the Tavus API at runtime — no role file is written to disk.

## 2. Configure the Persona

The persona — system prompt, objectives, guardrails, perception (Raven) queries, turn-taking (Sparrow) — is created and edited in the [Tavus dashboard](https://platform.tavus.io/personas) or via the Tavus API. The deployed persona is the source of truth; there are no local persona definition files. Point the preset's `persona_id` in `config/presets.config.json` at it (or run `npm run init` to copy a reference persona into your account and have its ID written for you). The app fetches the persona's objectives, guardrails, and tools live at runtime.
...
- The entry screen. The persona's objectives, guardrails, and tools are fetched in parallel via the `usePersonaObjectives` / `usePersonaGuardrails` / `usePersonaTools` hooks in `App.tsx` as the app boots.
...
- The replica appears and the audition begins
...
- The **FloatingInspector** always mounts as a right-docked, collapsible dev panel (portaled to `document.body`). It boots collapsed as a small terminal-style toggle so it never covers the replica; expanded, it docks full-height against the right edge with three zones: an always-visible Live State vitals strip (status / elapsed / speaker / objective progress / guardrail counter), an Inspector tab wrapping the `DeveloperInspector` (header → divider → columns: Objectives / Guardrails / Perception · Raven), and an Events tab (severity-colored CVI event console). Awareness streams into the Raven column as each `conversation.utterance` arrives. Open/closed state and active tab persist in localStorage under `ai-interviewer.dev-panel.v3`. An "Inject sample answer" button reads `persona/sample_answers.json` keyed by the active objective and sends a `conversation.respond` (dev-only convenience).
...
```bash
# Edit the persona in the Tavus dashboard, then point the preset at it
