...
The role lives on the preset (and, on the persona side, in the persona's system prompt). There is no separate role-config file in the repo.

## Adding/Removing Questions

Questions are defined as objectives on the persona — not in this repo's config files. Edit the persona's objectives in the [Tavus dashboard](https://platform.tavus.io/personas) (or via the Tavus API).
...
Guardrails are managed directly on the persona — via the Tavus dashboard or the [Guardrails API](https://docs.tavus.io/api-reference/guardrails/create-guardrails) — and the deployed persona is the source of truth. Create each guardrail (`POST /v2/guardrails`) and attach it to the persona via `guardrail_ids` / `guardrail_tags`. Each guardrail has:
- `guardrail_name` — unique identifier (alphanumeric + underscores)
- `guardrail_prompt` — the behavioral rule
- `modality` — `"verbal"` or `"visual"`

How the app *reacts* to a triggered guardrail (replica speaks / toast / inspector-only) is decided per guardrail in `src/lib/guardrailActions.ts`.

## Perception Queries

Perception queries live on the persona — edit them in the Tavus dashboard (or via the Tavus API). There is no local mirror.
...
Update the preset's `replica_id` in `config/presets.config.json` with the new replica ID from [platform.tavus.io/replicas](https://platform.tavus.io/replicas). This overrides the persona's `default_replica_id` for that preset.
